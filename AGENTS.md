...
- **交互与调用约定**：对用户的自然语言回复默认使用中文；调用大模型接口、检索或构造提示词时可使用英文，以提升工具调用和检索效果。
...
- **注释约定**：关键业务代码块与对外接口（Controller API、Service 公共方法、核心 Repository 查询）必须有简洁注释，至少说明意图、关键输入输出、边界/异常行为；禁止“翻译代码式”无效注释。
